...
public class SeamCarver3 {
    ...
    public SeamCarver3(Picture picture) {                // create a seam carver object based on the given picture
    ...
    public   int[] findHorizontalSeam() {               // sequence of indices for horizontal seam
        ...
        int block = 50;
        int parts = width / block;
        for (int p = parts; p >= 0; p--) {
        	for (int j = 0; j < height; j++) {
                findHorizontalSeamImpl(p * block, j, new int[width], resultSeam, cachedEnergySums, bestSeams);
            }
        }
        return resultSeam;
    }
    ...
    public   int[] findVerticalSeam() {                 // sequence of indices for vertical seam
        ...
        for (int p = parts; p >= 0; p--) {
        	for (int i = 0; i < width; i++) {
                findVerticalSeamImpl(i, p * block, new int[height], resultSeam, cachedEnergySums, bestSeams);
            }
        }
        return resultSeam;
    }

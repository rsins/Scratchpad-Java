...
public class Solution {
	...
	private static int getCountOfKDifferenceCombinations(int[] aNumbers, int aKDifference) {
		...
		if (aKDifference > (aNumbers[myArraySize - 1] - aNumbers[0])) {
			return 0;
		}

		int myCount = 0;
		for (int i1 = 0; i1 < (myArraySize-1); i1++) {
			if (Arrays.binarySearch(aNumbers, i1+1, myArraySize-1, aNumbers[i1]+aKDifference) > 0) {
				myCount++;
			} 
		}

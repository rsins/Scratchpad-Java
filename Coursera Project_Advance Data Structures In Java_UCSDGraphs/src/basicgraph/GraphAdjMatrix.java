...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] matrixMulResult = matrixMultiplication(adjMatrix, adjMatrix);
		for (int i = 0; i < getNumVertices(); i++) {
			for (int j = 0; j < matrixMulResult[v][i]; j++ ) {
				twoHops.add(i);
			}
		}
		return twoHops;
	...
	// Ignoring the size check. Assuming it will always be within the calculable criteria.

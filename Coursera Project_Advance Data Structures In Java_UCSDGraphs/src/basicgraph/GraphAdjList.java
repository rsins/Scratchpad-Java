...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 List<Integer> oneHops = adjListsMap.get(v);
		 for (Integer el : oneHops) {
			 if (adjListsMap.get(el) != null && adjListsMap.get(el).size() > 0) {
				 twoHops.addAll(new ArrayList<Integer>(adjListsMap.get(el)));
			 }
		 }
		 return twoHops;
	}
